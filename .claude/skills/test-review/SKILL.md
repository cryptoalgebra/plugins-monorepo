...
1. Run coverage for real numbers: `cd packages/<pkg> && npx hardhat coverage`. `@nomicfoundation/hardhat-toolbox` (which bundles `solidity-coverage`) is a single root devDependency shared by every package via the pnpm workspace, so this works from any package directory even though it isn't a package.json script. Read the terminal summary table (columns: `% Stmts`, `% Branch`, `% Funcs`, `% Lines`, `Uncovered Lines`, one row per contract) — this is the objective baseline, not a guess.
   - **A clean coverage run is now the expected result, so treat failures as findings rather than as background noise.** Every package with tests runs green under `hardhat coverage`; the instrumentation failures this repo used to take for granted were the `OnlyPool()` proxy-offset problem, and pinned bytecode closed it (see `test-add`'s `references/gotchas.md`). So when a test fails only under `coverage` and passes under plain `npx hardhat test`, do not write it off — it is usually one of two real problems: a fixture deploying a proxy from a fresh artifact instead of the pinned one, or a test that depends on execution order, because `.solcover.js` filters `@skip-on-coverage` out and so runs a different set of specs before yours. Both are worth reporting. Reserve `[COVERAGE-ARTIFACT]` for a case you have actually shown to be instrumentation, not for any coverage-only failure.
...
- **A data-driven corpus that only partly runs.** Where a suite expands recorded transactions from a JSON file into one case each, the loop may not reach every record, and it reports green either way. Two shapes: a runtime cap or stride over the array (none is left in the repo, but check), and a named exclusion set — `packages/alm/test/AlmPlugin.spec.ts` filters against `HELD_OUT`, records the contract reproduces differently. An exclusion is legitimate when the recording is not an oracle for correctness, but only if the excluded behavior is pinned somewhere else; a record silently dropped with nothing asserting what the contract now does with it is a finding. Say how many records are excluded, under which selection, and whether their behavior is covered elsewhere.
