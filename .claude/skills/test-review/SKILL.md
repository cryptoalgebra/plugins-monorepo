...
- **Inherited and composed behavior is often covered only incidentally.** What a plugin gets from the base layer — its pool wiring, the manager role, module registration, the delegatecall helper — may run only through whichever feature package's specs reach it, and a module's guard may be caught only by a package that composes it. When you credit such behavior as covered, name the spec doing the covering; when you cannot, that is a finding.
...
1. Run coverage for real numbers: `cd packages/<pkg> && npx hardhat coverage`. `@nomicfoundation/hardhat-toolbox` (which bundles `solidity-coverage`) is a single root devDependency shared by every package via the pnpm workspace, so this works from any package directory even though it isn't a package.json script. Read the terminal summary table (columns: `% Stmts`, `% Branch`, `% Funcs`, `% Lines`, `Uncovered Lines`, one row per contract) — this is the objective baseline, not a guess.
   - **A test that fails only under `coverage` is a finding, not noise.** It is usually a fixture deploying a proxy from a fresh artifact instead of pinned bytecode (the `OnlyPool()` symptom), or a test that depends on execution order, because `.solcover.js` filters `@skip-on-coverage` out and so runs a different set of specs before yours; both are in `test-add`'s `references/gotchas.md`. Reserve `[COVERAGE-ARTIFACT]` for a case you have actually shown to be instrumentation.
   - Don't let a coverage-run failure abort the review; the table is still written when tests fail. **Every contract those tests exercise is then understated in it** — a contract whose only tests failed can show 0% branches and uncovered lines that are in fact covered. Work out which rows the failing tests feed and say so instead of reporting those numbers as gaps.
   - **Quote numbers only from a clean build.** Coverage over build output that plain test runs have touched can report 0% for contracts whose tests pass. Delete the package's `artifacts`, `cache`, `coverage` and `coverage.json` first; all four are gitignored build output.
2. Read the contract(s) under `packages/<pkg>/contracts/` — every external/public function, revert condition, event, and access-control modifier is a candidate for a test. **Exclude everything under `contracts/test/`**, subdirectories included (mocks, `Mock*`, `*Test.sol` harness contracts) from this pass — they're test infrastructure, not production code, and their coverage numbers are not part of the target.
...
7. **Ask what would survive a mutation.** For each `require`, `emit`, comparison and arithmetic operator in a production contract, name the test that fails if it is deleted, moved to its neighbour (`<` to `<=`) or swapped (`+` to `-`). Coverage stays green over all of them; where no test would fail, that is a finding.
...
- Revert condition (`require`, custom error, `onlyOwner`-style modifier) never asserted with `.to.be.revertedWith(...)` / `.to.be.revertedWithCustomError(...)` / `.to.be.revertedWithoutReason()`. A bare `.to.be.reverted` does not count as covering it and is a `[WEAK]` finding in its own right: it passes for any revert, including one thrown before the guard under test was ever reached. Name the error the contract actually raises — and where the guard is a `require` with no reason string, `revertedWithoutReason()` is the precise assertion, which also fails if someone later adds a message. Where the error carries arguments, an assertion without a .withArgs(...) is the same finding one level down: it proves which error fired but not what it named.
- An `it` whose only assertion is that a call did *not* revert. This is the mirror of the bare `.to.be.reverted` above, and it needs the same judgement rather than a blanket rule: for the allowed half of an access-control pair, or for a module that is supposed to stay silent rather than throw, "the call went through" is exactly the claim and nothing more is owed. It is a `[WEAK]` finding only where the title promises something the assertion never looks at — a calculation, a preserved field, a value forwarded on. Read the title, then ask what the test would have to check for that title to be earned.
- Event never asserted with `.to.emit(...)`.
- An event or revert matcher with no `await` in front of `expect`, such as `expect(await call).to.emit(...)`. Nothing waits for the matcher, so the test passes whether the event fires or not: a `[WEAK]` finding, however exact its `.withArgs`.
- Untested boundary values (zero address, zero amount, max uint, empty arrays, same-value no-op calls), and a comparison never exercised at exactly its bound, where `<` and `<=` part ways.
...
- Upgrade and initialization paths. Measure an `Upgradeable*.spec.ts` against `test-add`'s minimum for that file, and a beacon-upgradeable plugin's `*.upgrade.spec.ts` against `test-add`'s upgrade checklist; a missing item is a finding. Absent storage-preservation coverage across an upgrade is a finding on its own — no amount of proxy-behavior testing reaches it.
...
- **A data-driven corpus that only partly runs.** Where a suite expands recorded transactions from a JSON file into one case each, the loop may not reach every record, and it reports green either way. Look for a cap or stride over the array, and for a named exclusion set of records the contract reproduces differently. An exclusion is legitimate when the recording is not an oracle for correctness, but only if the excluded behavior is pinned somewhere else; a record silently dropped with nothing asserting what the contract now does with it is a finding. Say how many records are excluded, under which selection, and whether their behavior is covered elsewhere.
- **A property test that passes without reaching what it guards.** Draws that die at the first guard, a latch an earlier run tripped, or a run count below what a planted fault needs all report green over lines other tests already cover. Check that a property writing state reloads its fixture and asserts a floor on how often the interesting path was reached; the shapes are in `test-add`'s `references/gotchas.md`.
...
Treat 100% line/branch coverage of `packages/<pkg>/contracts/` (excluding `contracts/test/`) as the baseline, and cite the actual percentage from the coverage run. But every finding still has to correspond to a real, distinct behavior — never propose a test whose only purpose is to touch a line without asserting anything new: no re-running an already-tested happy path with a behaviorally-equivalent input, no calling a trivial getter with no assertion, no duplicate `it` that exercises the same branch an existing test already exercises, and no chasing coverage on `contracts/test/` mocks.
...
A second class deserves the same treatment and is easier to miss, because the state *is* reachable: a branch whose two arms leave identical state. A guard that skips an addition of zero, an early return that the function it calls repeats anyway, or a value computed only for a return every caller discards, is a gas shortcut rather than a fork in behaviour, and a test aimed at it flips the counter and then passes unchanged when the condition is inverted. Ask what the other arm would leave behind before proposing the case; where the answer is "the same thing", report it with that reason rather than as a gap.
...
- Which of the proposed cases are actually writable with infrastructure that already exists? Check `contracts/test/` harnesses and `test-utils` before assuming a scenario is impractical — mocks whose state can be driven directly, swap/callee helpers, and math helpers exposing internals are usually already there, and a finding is much more likely to be acted on when it names the helper that makes it a short test. If a case genuinely needs new infrastructure, say so in the finding instead of dropping it.
...
[UNREACHABLE] <file>:<line> — <why it can't realistically be covered, or why both arms leave the same state>, so it's excluded from the target
[COVERAGE-ARTIFACT] <test name> — fails only under `hardhat coverage`; <what showed it is instrumentation, not a fixture or ordering fault>
```
