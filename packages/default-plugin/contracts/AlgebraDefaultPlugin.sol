...
/// @title Algebra Integral 1.2.1 sliding fee + oracle plugin
contract AlgebraDefaultPlugin is SlidingFeePlugin, VolatilityOraclePlugin {
  ...
  constructor(
    ...
    uint16 _baseFee
  ) BaseAbstractPlugin(_pool, _factory, _pluginFactory) SlidingFeePlugin(_baseFee) {}
  ...
  function beforeSwap(address, address, bool zeroToOne, int256, uint160, bool, bytes calldata) external override onlyPool returns (bytes4, uint24, uint24) {
    ...
    uint24 newFee = uint24(_getFeeAndUpdateFactors(zeroToOne, currentTick, lastTick));
