...
async function main() {
  ...
  const MevxImpl = await ethers.getContractFactory("MevxPluginImplementation");
  const mevxImpl = await MevxImpl.deploy();
  await mevxImpl.waitForDeployment();
  console.log("MevxImpl:", await mevxImpl.getAddress());
  ...
  const SlidingFeeImpl = await ethers.getContractFactory("SlidingFeePluginImplementation");
  const slidingFeeImpl = await SlidingFeeImpl.deploy();
  await slidingFeeImpl.waitForDeployment();
  console.log("SlidingFeeImpl:", await slidingFeeImpl.getAddress());
  ...
  // Set MevX defaults (optional)
  if (config.mevxRouter !== ZERO_ADDRESS) {
    const tx3 = await factory.setMevxRouter(config.mevxRouter);
    await tx3.wait();
    console.log("Set MevX router:", config.mevxRouter);
  }
  if (config.mevxExecutor !== ZERO_ADDRESS) {
    const tx4 = await factory.setMevxExecutor(config.mevxExecutor);
    await tx4.wait();
    console.log("Set MevX executor:", config.mevxExecutor);
  }
  if (config.profitDistributor !== ZERO_ADDRESS) {
    const tx5 = await factory.setProfitDistributor(config.profitDistributor);
    await tx5.wait();
    console.log("Set profit distributor:", config.profitDistributor);
  }
  if (config.mevxConfigId !== ZERO_BYTES32) {
    const tx6 = await factory.setConfigId(config.mevxConfigId);
    await tx6.wait();
    console.log("Set MevX configId:", config.mevxConfigId);
  }

  // Set FeeDiscountRegistry
  const tx7 = await factory.setFeeDiscountRegistry(await feeDiscountRegistry.getAddress());
  await tx7.wait();
  console.log("Set FeeDiscountRegistry");

  // Set LimitOrderManager
  const tx8 = await factory.setLimitOrderManager(await limitOrderManager.getAddress());
  await tx8.wait();
