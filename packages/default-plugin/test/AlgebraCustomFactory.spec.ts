...
import { AlgebraCustomPluginFactory } from '../typechain';
...
describe('AlgebraCustomPluginFactory', () => {
  ...
  let customPluginFactory: AlgebraCustomPluginFactory;
  ...
  beforeEach('deploy test volatilityOracle & custom deployer', async () => {
    ...
    factory = fix.factory;
    const customDeployerFactory = await ethers.getContractFactory('AlgebraCustomPluginFactory');
    customPluginFactory = (await customDeployerFactory.deploy(factory, fix.entryPoint)) as any as AlgebraCustomPluginFactory;
