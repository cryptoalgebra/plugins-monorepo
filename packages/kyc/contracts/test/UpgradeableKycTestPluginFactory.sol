...
contract UpgradeableKycTestPluginFactory is IBasePluginFactory, IKycPluginFactory {
  address public override kycIdentityFactory;

  uint256 public override kycRequiredTopic;
  ...
  function _createPlugin(address pool) internal returns (address plugin) {
    ...
    bytes memory initData = abi.encodeCall(UpgradeableKycPluginTest.initialize, (pool, kycIdentityFactory, kycRequiredTopic));
  ...
  function setKycIdentityFactory(address newKycIdentityFactory) external override {
    require(kycIdentityFactory != newKycIdentityFactory);
    kycIdentityFactory = newKycIdentityFactory;
    emit KycIdentityFactory(newKycIdentityFactory);
  }

  /// @inheritdoc IKycPluginFactory
  function setKycRequiredTopic(uint256 newKycRequiredTopic) external override {
    require(kycRequiredTopic != newKycRequiredTopic);
    kycRequiredTopic = newKycRequiredTopic;
    emit KycRequiredTopic(newKycRequiredTopic);
  }
}
