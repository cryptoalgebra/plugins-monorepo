...
import '@cryptoalgebra/abstract-plugin/contracts/interfaces/IAbstractPluginFactory.sol';
...
contract LimitOrderPlugin is ILimitOrderPlugin, LimitOrderPayments {
  ...
  modifier onlyPlugin(address pool) {
    address plugin = IAbstractPluginFactory(basePluginFactory).pluginByPool(pool);
