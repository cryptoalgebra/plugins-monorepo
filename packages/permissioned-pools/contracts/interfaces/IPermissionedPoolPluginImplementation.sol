...
interface IPermissionedPoolPluginImplementation {
  ...
  /// @notice Verify pool initialization — reverts unless at least one token has a checker assigned
  /// @param pool The pool address
  function verifyInitialize(address pool) external view;

  /// @notice Verify a swap for both tokens independently — reverts unless the resolved real
  /// sender's checker flags include SWAP_ALLOWED for each token that has a checker
  /// @param pool The pool address
  ...
  /// @notice Verify an add-liquidity operation — reverts unless the resolved real sender's checker
  /// flags include LIQUIDITY_ALLOWED for each token that has a checker
  /// @param pool The pool address
  /// @param sender The raw hook sender
