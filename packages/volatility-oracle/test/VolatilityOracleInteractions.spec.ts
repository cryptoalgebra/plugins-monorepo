...
describe('VolatilityOracleInteractions', () => {
  ...
  let oracleLibraryTest: VolatilityOracleInteractionsTest;
  ...
  const oracleLibraryTestFixture = async () => {
    ...
    const oracleLibraryFactory = await ethers.getContractFactory('VolatilityOracleInteractionsTest');
    const oracleLibrary = await oracleLibraryFactory.deploy();

    return {
      tokens: tokens as TestERC20[],
      oracleLibraryTest: oracleLibrary as any as VolatilityOracleInteractionsTest,
    };
  };
